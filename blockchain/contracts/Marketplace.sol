...
// This is a custom interface to allow the Marketplace to call the isVerified function
// on your PropertyTitle contract.
interface IPropertyTitle {
    // FIX: Changed uint264 to the correct uint256
    function isVerified(uint256 tokenId) external view returns (bool);
}

/**
 * @title Marketplace
 * @dev An advanced marketplace for ERC721 PropertyTitle tokens.
 * This version uses the secure "pull-payment" pattern for seller payouts
 * and includes features for canceling listings.
 */
contract Marketplace is ReentrancyGuard {
     ...
     * CRITICAL: It also checks if the property has been verified by the LRD.
     */
    function buyProperty(uint256 tokenId) external payable nonReentrant {
        ...
        // Use the custom interface to check the verification status
        require(IPropertyTitle(address(propertyTitle)).isVerified(tokenId), "Property has not been verified by the authority");
    ...
    function withdrawProceeds() external nonReentrant {
        ...
        // Reset the seller's balance to zero BEFORE sending the funds to prevent re-entrancy
